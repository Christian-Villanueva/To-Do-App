...
import Icon from './assets/To-Do.png'
import '@fortawesome/fontawesome-free/css/all.min.css';
import DateTimeCurrent from './assets/Components/DateTimeCurrent'
import DateTimePicker from './assets/Components/DateTimePicker'
import EditModal from './assets/Components/EditModal';
import DeleteConfirmation from './assets/Components/DeleteConfirmation';
...
function App() {
  ...
  const addTask = () => {
    if (!taskName || !dueDateTime || taskName.length === 0 || dueDateTime.length === 0) {
      setShowAlert(true);
  ...
  const handleModalSave = (updatedTask) => {
    if (updatedTask.task.trim().length === 0) {
        setShowAlert(true);
        setTimeout(() => setShowAlert(false), 5000);
        return;
    }
            ...
            {filteredTasks.map((element) => (
              element.completed && (
                <div className={0 % 2 === 0 ? 'doneTaskFrame-odd': 'doneTaskFrame-even'} key={element.id}>
                  ...
                  <div className="taskname-due">
                    <div className="col taskName"><strike>{truncateText(element.task,30)}</strike></div>
                    <div className="col deadline">
                      Due Date: {element.due}
                    </div>
                  </div>
